        ...
        <table class="table table-hover align-middle mb-0 text-center">
          ...
          <tbody>
            <tr v-for="t in transactions" :key="t.id">
              <td class="align-middle">{{ t.sender?.name }}</td>
              <td class="align-middle">{{ t.receiver?.name }}</td>
              ...
              <td class="align-middle fw-bold">{{ t.total_debited }}</td>
              <td class="align-middle">{{ new Date(t.created_at).toLocaleString() }}</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<script setup>
  ...
  watch(list, (newList) => {
      ...
      if (props.type === 'sent') {
      transactions.value = newList.filter(t => t.sender_id === user.value.id)
      } else if (props.type === 'received') {
      transactions.value = newList.filter(t => t.receiver_id === user.value.id)
      } else {
